...
impl Repository {
    ...
    fn pull(&self, path: &Path) -> Result<Index, Error> {
        ...
        repo.find_remote("origin")?.fetch(&[self.branch], None, None)?;
        ...
        let index = repo.merge_commits(&last_commit, &fetched_commit, Some(&MergeOptions::new()))?;
        Ok(index)
    }
    ...
    pub fn update(&self) { //add --force_rewrite_repos=true option(meaning it will rewrite git repos)
        ...
        if repo_path.exists() && repo_path.is_dir() {
            ...
            let _idx = match self.pull(repo_path) {
                Ok(idx) => idx,
                Err(err) => panic!("failed to pull: {}", err),
            };
        }
    }
